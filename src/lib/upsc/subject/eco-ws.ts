...
export class EcoWebSocket {
  ...
  public addMessageListener(listener: EcoEventListener): void {
  ...
  public configure(config: EcoConfig = {}): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.notifyError("Failed to configure Economics interview");
        reject(error);
      }
    });
  }
  ...
  public async startRecording(): Promise<void> {
    if (!this.isConfigured) {
      throw new Error("Economics interview is not configured. Call configure() first.");
    }
  ...
  public endInterview(): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      this.notifyMessage("Ending Economics interview...");
    } else {
