...
export class PolityWebSocket {
  ...
  public addMessageListener(listener: PolityEventListener): void {
  ...
  public configure(config: PolityConfig = {}): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.notifyError("Failed to configure Polity interview");
        reject(error);
      }
    });
  }
  ...
  public async startRecording(): Promise<void> {
    if (!this.isConfigured) {
      throw new Error("Polity interview is not configured. Call configure() first.");
    }
  ...
  public endInterview(): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      this.notifyMessage("Ending Polity interview...");
    } else {
