...
export type HistoryEventListener = (message: string) => void;

export class HistoryWebSocket {
  ...
  private onMessageListeners: HistoryEventListener[] = [];
  ...
  constructor(private serverUrl: string = "ws://localhost:8764") {}

  public addMessageListener(listener: HistoryEventListener): void {
  ...
  public configure(config: HistoryConfig = {}): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
        // console("WebSocket connected, sending History configuration");
          if (this.ws) {
        ...
        this.notifyError("Failed to configure History interview");
        reject(error);
      }
    });
  }
  ...
  public async startRecording(): Promise<void> {
    if (!this.isConfigured) {
      throw new Error("History interview is not configured. Call configure() first.");
    }
    ...
    // console("History recording started with correct audio parameters");
    } catch (error) {
  ...
  // console("History recording stopped");
  }
  ...
  // console("History microphone paused - audio transmission stopped");
  }
  ...
  public resumeAudio(): void {
    ...
    this.notifyStatusChange("recording");
  // console("History microphone resumed - audio transmission restarted");
  }

  public endInterview(): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify({ type: "END_INTERVIEW" }));
      this.notifyMessage("Ending History interview...");
    } else {
  ...
  public disconnect(): void {
    ...
    this.isConfigured = false;
    this.notifyStatusChange("disconnected");
  // console("Disconnected from History interview, all resources cleaned up");
  }
