...
export type CSATEventListener = (message: string) => void

export class CSATWebSocket {
  ...
  private onMessageListeners: CSATEventListener[] = []
  ...
  constructor(private serverUrl = "ws://localhost:8766") { }

  public addMessageListener(listener: CSATEventListener): void {
  ...
  public configure(config: CSATConfig = {}): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
        // console("WebSocket connected, sending initial CSAT configuration")
          if (this.ws) {
            // Send configuration without language or user info
            const initialConfig = {
          ...
          if (typeof event.data === "string") {
            try {
              ...
              // console("Language selection prompt received:", jsonData)
                // This is the exact message format we're expecting:
                // {"status": "language_selection", "message": "Would you like to conduct this interview in English or Hindi?", "options": ["English", "Hindi"]}
                ...
                const message = this.selectedLanguage === "hindi" ? jsonData.hindi : jsonData.english
        ...
        this.notifyError("Failed to configure CSAT interview")
        reject(error)
      }
    })
  }
  ...
  public selectLanguage(language: string): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      // Send language selection as a separate message with the correct format
      const languageMessage = {
        language: this.selectedLanguage,
      }

    // console("Sending language selection message:", languageMessage)
      this.ws.send(JSON.stringify(languageMessage))
    } else {
      console.error("Cannot send language preference: WebSocket connection not open")
      this.notifyError("Cannot select language: connection is not available")
    }
  }
  ...
  public async startRecording(): Promise<void> {
    if (!this.isConfigured) {
      throw new Error("CSAT interview is not configured. Call configure() first.")
    }
    ...
    // console("CSAT recording started with correct audio parameters")
    } catch (error) {
      console.error("Error starting CSAT recording:", error)
  ...
  // console("CSAT recording stopped")
  }
  ...
  // console("CSAT microphone paused - audio transmission stopped")
  }
  ...
  public resumeAudio(): void {
    ...
    this.notifyStatusChange("recording")
  // console("CSAT microphone resumed - audio transmission restarted")
  }

  public endInterview(): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify({ type: "END_INTERVIEW" }))
      this.notifyMessage("Ending CSAT interview...")
    } else {
  ...
  public disconnect(): void {
    ...
    this.isConfigured = false
    this.notifyStatusChange("disconnected")
  // console("Disconnected from CSAT interview, all resources cleaned up")
  }
