export class InterviewSocketClient {
    ...
    public connect(resumePdf: string, numberOfQues: number, difficulty: string): Promise<void> {
    ...
    public addQuestionAnswer(question: string, answerText: string): Promise<void> {
      return new Promise((resolve, reject) => {
        if (!this.socket) return reject("Socket not connected.");
        this.socket.send(
          JSON.stringify({
